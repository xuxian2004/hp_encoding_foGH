...
In this section, we show that \xxx{the context bisimilarity} in \HOPiDd\ can be characterized by the much simpler normal \xxx{bisimilarity}. % called normal bisimulation.
...
%\sep
\xxx{We focus on unary parameterization, i.e., abstractions and applications that admit only one variable or instance respectively, for the sake of simplicity.
Moreover, unary parameterization appears more elementary; for example,  a k-ary abstraction can be treated as a sequence of unary abstractions.
Also recall that the encodings in the previous sections actually utilize only unary parameterization. 
Intuitively, by means of polyadic communication \cite{SW01a}, the characterization by normal bisimilarity can be extended to arbitrary dimensions of parameterization. }
...
We first give the factorization theorem. % in presence of parameterization on names. % (and on processes as well). We recall that $\equiv$ is the structural congruence.
As explained in Section \ref{s:introduction}, the factorization theorem is the cornerstone of characterizing context \xxxx{bisimilarity} with normal \xxxx{bisimilarity}, and 
the upshot of establishing the factorization theorem is to find the right small processes so-called triggers. Here we have three kinds of triggers, to tackle different kinds of parameterization. In particular, we stipulate that the triggers are as follows:  $\rrc{\triggerd} \DEF \lrangle{z}\overline{m}[\lrangle{Y}(Y\lrangle{z})]$, $\rrc{\triggerD} \DEF \lrangle{Z}\overline{m}Z$, and $\rrc{\trigger} \DEF \overline{m}$. These triggers are of somewhat a similar flavour but quite different in shape, with the aim at factorizing out respectively a name abstraction, a process abstraction and a non-abstraction process in some context. The first trigger, i.e.,  $\triggerd$, is the main contribution of this work, whereas the other two are inherited from \cite{Xu13} and \cite{San92} respectively.
\xxx{We note that $\ve{\lrangle{U}}E'$ denotes a series of (unary) abstraction on $E'$. %i.e., stands for $\lrangle{U_1}\lrangle{U_2}\cdots \lrangle{U_k} E'$.
}
...
\item[(1)] if $A$ is not an abstraction, then
\begin{itemize}
\item[(i)] if $E[\trigger]$ is $\ve{\lrangle{U}}E'$ for some non-abstraction $E'$, then  \\ %(here and below, $\ve{\lrangle{U}}$ denotes the abstractions prefixing $E'$), then \\
$E[A] \WCB \ve{\lrangle{U}}((m)(E' \para  !m.A))$;
\item[(ii)] %particularly, 
if $E[\trigger]$ is not an abstraction, then \\
$E[A] \WCB (m)(E[\trigger] \para  !m.A)$.
\end{itemize}
...
\item[(ii)] %particularly, 
if $E[\triggerD]$ is not an abstraction, then \\
$E[A] \WCB (m)(E[\triggerD] \para  !m(Z).A\lrangle{Z})$.
\end{itemize}
...
%---------------another way of stating END ------------

%\begin{proof}
\xxxx{In Theorem \ref{factor-bigd-smalld}, we explicitly distinguish between the parameterized case and non-parameterized case, and state them in separate clauses so as to emphasize their difference in forms and allow convenience in related discussion, though certainly it could be stated in a more uniform way \cite{San92,SW01a}, as follows. %, though in effect they are the same.
%---------------another way of stating BEGIN ------------
%\begin{theorem}[Factorization]\label{factor-bigd-smalld-simp} %[Factorization]
\begin{itemize}
\item[]
Given $\ve{\lrangle{U}}E[X]$ in which $E$ is not an abstraction, it holds for every $A$, fresh $m$ (i.e., $m\notin \fn{E,A}$) that: 
%\begin{itemize}
%\item[(1)] 
(1) if $A$ is not an abstraction, then 
$
\ve{\lrangle{U}}E[A] \WCB \ve{\lrangle{U}}((m)(E[\trigger] \para  !m.A))
$. ~
%\item[(2)] 
(2) if $A$ is a process abstraction, then 
$
\ve{\lrangle{U}}E[A] \WCB \ve{\lrangle{U}}((m)(E[\triggerD] \para  !m(Z).A\lrangle{Z}))
$. ~
%\item[(3)] 
(3) if $A$ is a name abstraction, then 
$
\ve{\lrangle{U}}E[A] \WCB \ve{\lrangle{U}}((m)(E[\triggerd] \para  !m(Z).Z\lrangle{A}))
$.
%\end{itemize}
\end{itemize}
%\end{theorem}
...
Clause (1) of Theorem \ref{factor-bigd-smalld} is actually Sangiorgi's seminal work \cite{San92, SW01a}. Clause (2) is shown in \cite{Xu13}. Clause (3) depicts the factorization for the abstraction on names, and is the contribution of this work.   %, can be discussed through a resembling technical routine. % almost the same as (ii). %and (i)
Intuitively, it allows one to replace, in a context $E$, a specific component $A$ (being a name abstraction) with the uniform trigger $\triggerd$, move $A$ into a parallel new place, and in the meanwhile build up a connection between $\triggerd$ and the new place, so as to maintain the invariance of observational behaviour.
...
As opposed to the original process, the  process after factorization has a lighter structure to discuss, and one can concentrate on the new place where the factorized process might exhibit potentially distinct behaviour.
...
With regard to the method of \emph{trigger} (including the technical approach), the fundamental framework is well-developed in the field, due to Sangiorgi \cite{SW01a}. The key to   establishing the factorization property for processes allowing abstraction on names is the \emph{trigger}, which had been unknown for a long time in contrast to the case of abstraction on processes and that without abstractions. 
\xxx{Once a right trigger is found, the remainder of discussion is relatively more tractable, though still complicated at some points.}
%can be made through a similar (but still tricky at some point) technical routine.

Theorem \ref{factor-bigd-smalld} depends on some distribution property concerning the process on the move. We put the proof of that property and  the proof of Theorem \ref{factor-bigd-smalld} in Appendix \ref{a:proofs-normal}. % with an example.
%\begin{example}
Below we give an example of the factorization concerning abstraction on names. 

%\noindent\textbf{Example} 
\begin{example}\label{eg:fac_smalld1}
The basic idea of factorization concerning abstraction on names can be illustrated in the following example in which $m$ is fresh (i.e., not in $A\lrangle{\rrc{d}}$).
%\[
\begin{eqnarray}
...
\subsection{Normal bisimulation for \HOPiDd}
Now we give the definition of normal bisimulation whose clauses are designed on top of the factorization theorem. We recall that $\rrc{\trigger} \DEF \overline{m}$, $\rrc{\triggerD} \DEF \lrangle{Z}\overline{m}Z$, and $\rrc{\triggerd} \DEF \lrangle{z}\overline{m}[\lrangle{Y}(Y\lrangle{z})]$.
...
The strong version of \WNB\ is denoted by \SNB, which is obtained by replacing above every double arrow with a single arrow and $\wt{}$ with $\st{\tau}$ in particular.
\end{definition}

Normal bisimilarity is a congruence, as stated in the following theorem. %with routine proof; 
\begin{theorem}\label{thm:normal_congru}
Relation $\WNB$ is a congruence \xxx{w.r.t. parallel composition and restriction}.
\end{theorem}
%The proof is similar to those in the references \cite{San92,SW01a,Xu13,DHS18}.
\xxxx{The proof is similar to the congruence proofs in \cite{San92,SW01a,Xu13,DHS18}. 
The framework of proving congruence in the references can be reused. Interestingly, this framework somewhat reminds one of the up-to  technique \cite{DHS18}.
} 
%We note that although we have a new form of triggers here, the proof approach is exactly the same. Intuitively, this is because the trigger's specific form does not matter when proving the congruence, as long as it is uniformly used, so the framework of proving congruence in the references can be reused. Interestingly, this framework appears to somewhat remind one of the up-to context technique \cite{DHS18}. 
\xxxx{To be complete, we give the proof of Theorem \ref{thm:normal_congru} in Appendix \ref{s:normal}.
}
\xxxx{We also note that in much the same way as Definition \ref{d:up2} and Theorem \ref{thm:sound_up-to_context_general}, we can define normal bisimulation up-to (context) and show its validity. }

\ntsoorm{
	NTing: 
	The proof of the congruence of normal bisimilarity may need to use the technique of up-to restriction (or up-to context). That is, in the closure of parallel composition, say $P\para O \WNB Q\para O$ from $P\WNB Q$ (and we define the relation as $\R\DEF\{(P\para O, Q\para O) \,|\, P\WNB Q\}$), the case when $P$ and $O$ incurs a communication, or more accurately the case when $P$ makes an input $a(A)$ while $O$ makes an output $(\ve{c})\overline{a}A$, the closing of bisimulation may involve the handling of the restricted names carried with the output by $O$, so this situation may need up-to restriction (or up-to context). To be more specific, the interaction 
	\[
    ...
    \] Now we only have $P'\para O' \,\R\, Q'\para O'$ because $P'\WNB Q'$. This is the time up-to restriction (i.e., $\ve{c}$ in this case, or context as $(\ve{c})([\cdot])$) comes into play.\\
...
\subsection{Coincidence between normal bisimilarity and context bisimilarity in \HOPiDd}
...
\xxxx{
Obviously $\WCB$ is finer than $\WNB$ because it requires no less than $\WNB$. 
In particular, $\WNB$ can be considered as a special case of $\WCB$ when the input and the receiving environment of output are chosen to be as in $\WNB$.
So we only need to prove that $\WNB$ implies $\WCB$.
}
To do this, we define the relation $\mathcal{R}$ as follows and show that it is a context bisimulation using Theorem \ref{factor-bigd-smalld}. % (\stress{up-to sth?}). 
\[
\mathcal{R} \DEF \{(P,Q) \,|\, P\WNB Q \}
\]
\xxx{Suppose $P\,\mathcal{R}\, Q$. 
There are several cases %(with subcases) 
to analyze. We focus on the major ones, %(as compared to related works \cite{San92, Xu13}), %
that is, the cases when $P$ %(or $Q$) 
communicates a name abstraction 
%and clause (3)(ii) of Theorem \ref{factor-bigd-smalld} applies, 
and when Theorem \ref{factor-bigd-smalld} applies for some non-abstraction $E$, 
%and $\ve{\lrangle{U}}$ is empty, 
%and leave out the similar (and simpler) rest.
and leave out the other cases.
}
