...
\begin{theorem}
Relation $\WNB$ is a congruence.
\end{theorem}
The proof is similar to those in the references \cite{San92,SW01a,Xu13,DHS18}. We note that although we have a new form of triggers here, the proof approach is exactly the same. Intuitively, this is because the trigger's specific form does not matter when proving the congruence, as long as it is uniformly used, so the framework of proving congruence in the references can be reused. Interestingly, this framework appears to also exploit the up-to context technique \cite{DHS18}.
\ntsoorm{
	NTing: 
