...
\begin{theorem}[Soundness of the up-to technique]
If \R is a context bisimulation up-to context or/and up-to $\SCB$ (or $\equiv$), then $\R \,\subseteq\, \WCB$.
\end{theorem}
}

\xxrmcolor{
Intuitively, the up-to techniques are sound because higher-order processes can be translated into first-order ones in a fully abstract way, and these up-to techniques are sound for first-order processes \cite{San92, SW01a, MPS14}. % (which we may also appeal to while discussing the encodings). 
