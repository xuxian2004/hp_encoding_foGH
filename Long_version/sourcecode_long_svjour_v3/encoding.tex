...
The encoding has its core defined as below, and is homomorphic on the other operators. %, and we note that the encoding of input guarded replication is defined as $\enc{!m(x).P}\DEF !\enc{m(x).P}$.
\[
\begin{array} {lrcll}
...
\end{array}
\]
The encoding uses both name parameterization and process parameterization. %, and seems correct. 
...
The last equivlence is due to the name invariance property to be given shortly in Lemma \ref{l:syn-pro-encoding}.
% \[
% \begin{array}{lcl}
...
Apparently the encoding is compositional and divergence-reflecting (since the encoding does not introduce any extra internal action), preserves the (free) names and structural congruence,  as stated in the follow-up lemma whose proof is standard induction. %We stress that the side conditions in the encoding is not essential.
\begin{lemma}\label{l:syn-pro-encoding}
Assume $P,Q$ are \FOPi\ processes. The encoding above from \FOPi\ to \HOPiDd\ is compositional and divergence-reflecting; moreover $\enc{P}\fosub{n}{m} \equiv \enc{P\fosub{n}{m}}$ (called name invariance), and $\enc{P}\equiv \enc{Q}$ iff $P\equiv Q$.
\end{lemma}
%\sep
...
\xxrmcolor{Literally in the meanwhile, the operational correspondence actually says that the encoding of a process always evolves into the encoding of another, thus forming a somewhat closed image domain (clause (2) is irrelevant to this point since its input does not come from an encoding, it is provided for the sake of enabling observation from a different angle).}
...
In Lemma \ref{l:opcor} and Lemma \ref{l:opcor-conv}, the input cases are not stated for any input process, because the action correspondence would become unclear if arbitrary input is considered, due to the somewhat richer environment of \HOPiDd. This is one reason why we argue for relinquishing uniform requirement on operational semantics in the notion of encoding (Section \ref{s:criteria}). These two lemmas can be proven in a similar fashion, %(we give details in Appendix \ref{a:proofs-encoding} \nts{\fbox{MOVE the proof here?!}}),
...
\xxrmcolor{
We have the following lemma (Lemma \ref{l:soundness}) stating the weak soundness of the encoding. We recall that $\WWCB$ is the $\WCB$ restricted to the image of the encoding as defined in Section \ref{s:preliminary}; or in other words context bisimulation defined over the image of the encoding. It should be clear that $\WWCB$ is well defined, due to the operational correspondence and static properties of the encoding. A crucial point here, thanks to the tight operational correspondence (by `tight', we mean that an encoding \HOPiDd\ process always evloves into the encoding of another \FOPi\ process, and thus rendering the restriction to the encodings sensible),  is that the contexts to be considered in the bisimulation are those processes in the target model that have reverse-image w.r.t. the encoding. 
}

\xxrmcolor{We note that the proof uses the standard up-to technique to establish bisimulation \cite{San94, Mil89, SW01a,PS11,San11}. We remember that if a relation $\mathcal{R}$ is a bisimulation up-to, say, $\equiv$, then in this relation the two simulating processes after making some action need not be directly related by $\mathcal{R}$, but (respectively) structural congruent to a pair of processes that belongs to $\mathcal{R}$. 
...
The completeness of the encoding is stated in the lemma below. {We note that completeness is true even if we do not constrain the domain to be the image of the encoded \FOPi\ processes. In terms of the notion of encoding, this is a more desired result. }
...
By Lemma \ref{l:opcor}, $\enc{P} \wt{\overline{a}[\lrangle{Z}(Z\lrangle{b})]} T$ and $T\SSEQV \enc{P'}$. Since $\enc{P} \WCB \enc{Q}$, we know that $\enc{P}$ must be able to be matched by $\enc{Q}\wt{\overline{a}[\lrangle{Z}(Z\lrangle{b})]} T'$, because $\enc{Q}$ can only output such shape of processes, and if the matching is, e.g., $\enc{Q}\wt{\overline{a}[\lrangle{Z}(Z\lrangle{c})]} T''$ then $\enc{P}$ and $\enc{Q}$ would be immediately distinguishable  
\xxrmcolor{(a %parallel 
context can be designed to distinguish between $\enc{P}$ and $\enc{Q}$ by harnessing $c$ to exhibit different behavior, e.g., $a(X).(X\lrangle{A})$ in which $A\DEF \lrangle{x}\overline{x}[\overline{d}]$ where $d$ is fresh) 
}.
...
So for every $E[X]$, we have $T \para E[\lrangle{Z}(Z\lrangle{b})] \WCB T' \para E[\lrangle{Z}(Z\lrangle{b})]$. By Lemma \ref{l:opcor-conv}, $Q \wt{\overline{a}b} Q'$ and $T'\SSEQV \enc{Q'}$. So we know
...
By Lemma \ref{l:opcor}, $\enc{P} \wt{(b)\overline{a}[\lrangle{Z}(Z\lrangle{b})]} T$ and $T\SSEQV \enc{P'}$. Since $\enc{P} \WCB \enc{Q}$, we know that $\enc{P}$  must be able to be matched by $\enc{Q}\wt{(b)\overline{a}[\lrangle{Z}(Z\lrangle{b})]} T'$ (apply $\alpha$-conversion if needed). This is because $\enc{Q}$ can only emit such form of processes, and moreover if the matching does not have a bound name (e.g., $\enc{Q}\wt{\overline{a}[\lrangle{Z}(Z\lrangle{c})]} T''$) then $\enc{P}$ and $\enc{Q}$ would be immediately distinguishable. %one can design a context to distinguish $\enc{P}$ and $\enc{Q}$. 
