...
\xx{Literally in the meanwhile, the operational correspondence actually says that the encoding of a process always evolves into the encoding of another, thus forming a somewhat closed image domain (clause (2) is irrelevant to this point since its input does not come from an encoding, it is provided for the sake of enabling observation from a different angle).}
...
\xx{(a context can be designed to distinguish between $\enc{P}$ and $\enc{Q}$ by harnessing $c$ to exhibit different behavior, e.g., $a(X).(X\lrangle{A})$ in which $A\DEF \lrangle{x}\overline{x}[\overline{d}]$ where $d$ is fresh) 
}.
