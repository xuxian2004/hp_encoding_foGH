...
Let $P_1\DEF (m)(\alpha.E[\triggerd] \para  !m(Z).Z\lrangle{A})$ and \\
$Q_1\DEF \alpha.(m)(E[\triggerd] \para  !m(Z).Z\lrangle{A})$. We define $\mathcal{R}_1 \DEF \{(P_1,Q_1)\}\cup \mathrm{ID}$ in which $\mathrm{ID}\DEF \{(P,P)\}$ is the identity relation. We show that $\mathcal{R}_1$ is a strong context bisimulation up-to $\equiv$, which forces the result to be true.  We iterate the cases on $\alpha$, which is obviously the only action $P_1$ incurs. %(the situation of the other way is similar).
\begin{itemize}
...
\[
\begin{array}{l}
B_1\equiv E_2[\triggerd] \st{(\ve{c})\overline{m}A'} B_1' \qquad A'\equiv \lrangle{Y}(Y\lrangle{h}) \;\;(\mbox{actually $\ve{c}$ only has $h$ in this case}) \\
