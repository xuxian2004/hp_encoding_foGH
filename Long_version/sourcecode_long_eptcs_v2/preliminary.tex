...
%\end{array}
%\]
Actions, ranged over by $\lambda,\alpha$, comprise internal move $\tau$, and visible ones: input ($a(b)$), output ($\overline{a}b$) and bound output ($\overline{a}(c)$ in which $c$ is bound). We note that actions occur only on name constants, and a communicated name is also a constant. % (i.e. no variable can be transmitted).
...
$\tau.P$ for $(c)(c.P\para \overline{c}.0)$ ($c$ fresh), and sometimes $\overline{u}[A].T$ for $\overline{u}A.T$ for the sake of clarity. Like \FOPi, a tilde represents a tuple.
...
we make some remarks about bisimulation proof method, since it is needed from time to time in the discussion of this work. 
}

\xx{
%NT: about up-to. ADD to after the definition of context bisimulation or another better place. 
The proofs concerning establishing bisimulation in this paper take advantage of a proof technique called up-to technique \cite{SW01a,PS11}. The up-to technique is somewhat a standard and universal tool for process models (even sequential models like lambda-calculus). It has been widely validated and applied. Interested readers can refer to \cite{SW01a,PS11,San12} for good details on this (traditional) technique. We will use the up-to technique in, for instance, the discussion of the properties of the encodings. 
}

\xx{
The up-to techniques used in this work mainly include up-to strong bisimulation (e.g., $\SCB$) or structural congruence ($\equiv$), and up-to context. 
Technically, a relation \R is a context bisimulation up-to $\SCB$ (respectively, $\equiv$) if it satisfies all the clauses of the context bisimulation, except that the resulting two processes in the conclusion of each clause are related by $\SCB \R \SCB$ (respectively, $\equiv \R \equiv$) instead of \R, i.e., replace \R with $\SCB \R \SCB$ (respectively, $\equiv \R \equiv$) in the conclusion of each clause. 
}

\xx{
Similarly, a relation \R is a context bisimulation up-to context if it satisfies all the clauses of the context bisimulation, except that the resulting two processes related by \R, say $P' \R Q'$, in the conclusion of each clause can be written as $P'\equiv C[P_1], Q'\equiv C[Q_1]$ for some $C,P_1,Q_1$ such that $P_1 \R Q_1$. That is, they can be rewritten into forms that extract the common parts and separate the possibly different sub-processes $P_1,Q_1$ to be indeed related by \R. 
}

\xx{
A relation \R as such above is not necessarily a bisimulation already, however it is certain that it is subsumed by the largest bisimulation (say, context bisimilarity). For example, if a relation \R is a context bisimulation up-to $\SCB$, the it holds that $\R \subseteq \WCB$. Briefly, this is true because  $\SCB \R \SCB$ is a context bisimulation. One has a similar situation for up-to context. We do not give further details since it is not the main topic of this work; systematic introduction and comprehensive discussion can be found in \cite{SW01a,SR12,San12,Mil89}. 
}
