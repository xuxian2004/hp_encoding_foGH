...
\xiv{We give the proof of Lemma \ref{l:opcor} in Appendix \ref{appendix:opcorres_encoding}.}
In these lemmas, 
...
\xiv{To be complete, we write down the precise definition below.} 

\xiv{
Realtion $\WWCB$ is the largest context bisimulation $\mathcal{R}$ on $\enc{\pi}$ that satisfies the following properties whenever $P\,\mathcal{R}\, Q$.
\begin{itemize}
\item if $P \st{\alpha} P'$ and $\alpha$ is $a(A)$ (in which $A$ is in $\enc{\pi}$) or $\tau$, then $Q \wt{\widehat{\alpha}} Q'$ for some $Q'$ and $P'\,\mathcal{R}\, Q'$;
...
\end{equation}
\end{itemize}
Notice that in the definition the context $E[X]$ in the output case might not itself be an encoding, %(for an example, see the context used for the counterexample in Section \ref{s:encoding_variant}.)
but indeed is required to become a process in range after evaluation with $A$ and $B$ respectively, i.e., $E[A]$ and $E[B]$ and thus the two processes in (**) are in $\enc{\pi}$.
See also the explanation in Section \ref{s:criteria}. %ttttt
}
\end{definition}
...
\xiv{Suppose $\enc{P}\,\mathcal{R}\, \enc{Q}$ and notice that we consier closed processes as default. 
}
We make a case analysis. %, where  Lemma \ref{l:opcor} and Lemma \ref{l:opcor-conv} %(as well as their weak versions)
...
{
\begin{lemma}\label{l:inter4completeness}
Suppose $P$ and $Q$ are \FOPi\ processes, and $\enc{P} \WCB \enc{Q}$. If we have an action $\enc{P} \wt{\overline{a}[\lrangle{Z}(Z\lrangle{b})]} T$ (respectively $\enc{P} \wt{(b)\overline{a}[\lrangle{Z}(Z\lrangle{b})]} T$), then $\enc{P}$ must be matched by $\enc{Q}\wt{\overline{a}[\lrangle{Z}(Z\lrangle{b})]} T'$ (respectively $\enc{Q}\wt{(b)\overline{a}[\lrangle{Z}(Z\lrangle{b})]} T'$).
\end{lemma}
}
\begin{proof}
{
We only prove the output in which $b$ is not bound, and the other case is similar.
First of all, $\enc{Q}$ can only output such shape of processes (by Lemma \ref{l:opcor}).
\xxxx{{We claim that if the matching is, say $\enc{Q}\wt{\overline{a}[\lrangle{Z}(Z\lrangle{c})]} T''$ 
(similar for the case $\enc{Q}\wt{(b)\overline{a}[\lrangle{Z}(Z\lrangle{b})]}$), then $\enc{P}$ and $\enc{Q}$ would be immediately distinguishable.   
\xxrmcolor{
In particular, a %parallel 
context can be designed to distinguish between $\enc{P}$ and $\enc{Q}$ by harnessing $c$ to exhibit different behaviour with some singular process; say, $H\DEF a(X).(X\lrangle{A})$ in which $A\DEF \lrangle{x}\overline{x}[\overline{d}]$ where $d$ is fresh. When putting $H$ in parallel and counteracting some interfering factors up-to structural equivalence (if necessary), we can see that $\enc{P}$ and $\enc{Q}$ would trigger output of fresh $d$ on different names (i.e., $b,c$), causing neither of them to be able to match the other.
}
}}
%the bound output case is similar and omitted.
%This is because $\enc{Q}$ can only emit such form of processes, and moreover if the matching does not have a bound name (say $\enc{Q}\wt{\overline{a}[\lrangle{Z}(Z\lrangle{c})]} T''$) then $\enc{P}$ and $\enc{Q}$ would be immediately distinguishable \xxx{for the reason similar to that of the last case}. %one can design a context to distinguish $\enc{P}$ and $\enc{Q}$. 
\myqed
}%color
\end{proof}
...
Suppose $P$ and $Q$ are \FOPi\ processes. Then $\enc{P} \WCB \enc{Q}$ implies $P\WGB Q$.
\end{lemma}
\begin{proof}
\tdup{
...
By Lemma \ref{l:opcor}, $\enc{P} \wt{\overline{a}[\lrangle{Z}(Z\lrangle{b})]} T$ and $T\SSEQV \enc{P'}$. Since $\enc{P} \WCB \enc{Q}$, we know that $\enc{P}$ must be matched by $\enc{Q}\wt{\overline{a}[\lrangle{Z}(Z\lrangle{b})]} T'$, \xiv{by Lemma \ref{l:inter4completeness}}.
...
By Lemma \ref{l:opcor}, $\enc{P} \wt{(b)\overline{a}[\lrangle{Z}(Z\lrangle{b})]} T$ and $T\SSEQV \enc{P'}$. Since $\enc{P} \WCB \enc{Q}$, we know that $\enc{P}$  must be able to be matched by $\enc{Q}\wt{(b)\overline{a}[\lrangle{Z}(Z\lrangle{b})]} T'$ (apply $\alpha$-conversion if needed), \xiv{by Lemma \ref{l:inter4completeness}}. 
% This is because $\enc{Q}$ can only emit such form of processes, and moreover if the matching does not have a bound name (say $\enc{Q}\wt{\overline{a}[\lrangle{Z}(Z\lrangle{c})]} T''$) then $\enc{P}$ and $\enc{Q}$ would be immediately distinguishable \xxx{for the reason similar to that of the last case}. %one can design a context to distinguish $\enc{P}$ and $\enc{Q}$. 
...
\xiv{(As a matter of fact, this could be made somewhat more precise by setting, for instance, $E$ to be $X\lrangle{\lrangle{x}R'}$, where $R'$ does not contain $b$, so that the implication from (\ref{eq:comp1}) to (\ref{eq:comp2}) follows by taking $R'\fosub{b}{x}$ as $\enc{R}$.)}. %is such that R'\{b/x\} = [[R]].)}
