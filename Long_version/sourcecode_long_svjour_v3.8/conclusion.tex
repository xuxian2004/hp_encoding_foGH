...
In this paper, we have exhibited two new encodings of name-passing in the higher-order paradigm that allows parameterization, and a normal bisimulation in that setting as well. In the former, we demonstrate the conformance or inconsistency of the encoding with respect to some well-established criteria in the literature. In the latter, we prove the coincidence between normal \xxxx{bisimilarity} and context \xxxx{bisimilarity} by pinpointing how to factorize an abstraction on some name. 
\xxx{
As a supporting byproduct, we show that the bisimulation up-to technique, particularly up-to context, works in the higher-order process model with parameterization.
}
...
We expect to have a factorization property like the one as follows: 
\begin{equation}\label{eq:fac_smdonly_eg1}
(\ve{c})(T\lrangle{d} \para F[A]) \WCB A\lrangle{d} \equiv \overline{d}
\end{equation} % (here $E[X]$ is $X\lrangle{d}$), 
%where $T$ is supposed to be a proper uniform-looking `trigger' (subject to the capability of the calculus), 
\xxxx{where $T$ is supposed to be some `trigger' subject to the capability of the calculus, }
and $F$ to be the new place holding a repository of $A$, with $\ve{c}$ being the local names shared by $T$ and $F$. %and by (\ref{eq:normal_pid_3})  %
...
Now the left hand side \xxx{of (\ref{eq:fac_smdonly_eg1})} must have a transition $\wt{\overline{d}}$, which should result from the interaction between $T\lrangle{d}$ and $F[A]$, and during these interactions $d$ must be received by $F$ so as to be fed to $A$ in its own setting such that $A$ eventually gets the right instantiation \xxx{in some uniform way}. 
...
To this point, resolving this conundrum may amount to exploiting further the expressiveness of the parameterization on name; or otherwise, showing the impossibility of characterizing context \xxxx{bisimilarity} should rest on some precisely formulated criteria. This might take us one step further to scrutinize the discrepancy in the role of names in higher-order models.
