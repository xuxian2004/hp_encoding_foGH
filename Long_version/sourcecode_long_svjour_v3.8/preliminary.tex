...
Respectively $\fn{\cdot}, \bn{\cdot}, \n{\cdot}, \fnv{\cdot}, \bnv{\cdot}, \nv{\cdot}$ denote free name constants, bound name constants, names, free name variables, bound name variables, and name variables in a set of processes or actions (to be defined shortly). A name is fresh if it does not appear in any process under discussion. 
%Usually closed processes are considered by default, i.e., those having no free variables.
\xxx{
Processes having no free variables are closed. Usually for convenience, closed processes are considered by default.	
}
In the standard way, here are a few derived operators: $\overline{m}(d).P \DEF (d)\overline{m}d.P$, $m.P \DEF  m(x).P \;(x\notin \fnv{P})$, $\overline{m}.P \DEF \overline{m}(d).P\;(d\notin \fn{P})$; $\tau.P\DEF (a)(a.P\para \overline{a}.0)$ ($a$ fresh). A trailing $0$ process is often omitted.
We denote tuples by a tilde. For tuple $\ve{n}$, $\size{\ve{n}}$ denotes its length and %$m\in \ve{n}$ means $m$ is its element;
...
The semantics of \FOPi\ is defined by the standard LTS (Labelled Transition System) below. \nsepvs{.2} %(symmetric rules are skipped).
\[
\begin{array}{lll}
...
\infer[{\scriptstyle c\not\in \n{\lambda}}]{(c)P\st{\lambda} (c)P'}{P\st{\lambda} P'} \quad &
...
\infer[\xxxx{b\notin \fn{P}}]{P\para Q\st{\tau}(b)(P'\para Q')}{P\st{a(b)} P'\quad Q\st{\overline{a}(b)} Q'} \quad
\end{array}
\]
% \[
% \begin{array}{ll}
...
%\end{array}
%\]
Actions, ranged over by $\lambda,\alpha$, comprise invisible internal move $\tau$, and visible ones: input ($a(b)$), output ($\overline{a}b$) and bound output ($\overline{a}(c)$ in which $c$ is bound; \xxxx{every name elsewhere in a label is free}). We note that actions occur only on name constants, and a communicated name is also a constant. % (i.e. no variable can be transmitted).
...
We denote by $\equiv$ the standard structural congruence \cite{MPW92}\cite{SW01a}, which is the smallest relation satisfying $\alpha$-convertibility, the monoid laws for parallel composition, commutative laws for both composition and restriction, and a distributive law $(c)(P\para Q) \SE (c)P\para Q$ (if $c\notin \fn{Q}$).
\xxxx{The structural congruence is not part of the semantics, and we use it only as some structural manipulation method.}
\xxxx{With the help of $\alpha$-conversion, we always assume no name capture (which means that a free name falls into the scope of a same bound name). }
...
If $P\st{\lambda} P'$, %where $\lambda$ is $a(b)$, $\overline{a}b$, $\overline{a}(b)$, or $\tau$, 
then $Q\wt{\hat{\lambda}} Q'$ for some $Q'$ and $P'\,\mathcal{R}\, Q'$.
%\end{itemize}
...
We denote by $\SGB$ the strong \ground bisimilarity (i.e., replacing $\wt{\widehat{\alpha}}$ with  $\st{\alpha}$ in the definition). 
\xxx{
Sometimes when it causes no confusion, we will use bisimulation instead of bisimilarity so as to allow more flexibility in some paraphrasing. 	
}
It is well-known that $\WGB$ is a congruence \cite{EN00,SW01a}, and coincides with the so-called local bisimilarity as defined below \cite{Fu05b,Xu12}.
...
We use $u.0$ for $u(X).0$, $\overline{u}.0$ for $\overline{u}0.0$, %$\overline{m}A$ for $\overline{m}A.0$;
 ...
 Same as in \FOPi, a tilde represents a tuple.
A higher-order substitution $T\hosub{A}{X}$ replaces variable $X$ with $A$ and can be extended to tuples in the usual way.
\xxx{
Contexts are defined also like in \FOPi. 
In particular, contexts can be extended to multihole ones in the standard way \cite{SW01a}; basically a multihole context has several holes each of which may occur a couple of times. By default, we consider one-hole contexts.
\xxxx{To facilitate bisimulation and related discussion, we denote by
$E[\ve{X}]$ (sometimes called a environment) the expression $E$ (possibly) with free variables $\ve{X}$, and $E[\ve{A}]$ stands for $E\hosub{\ve{A}}{\ve{X}}$ \xxxx{respecting name-capture}. 
A context $C[\cdot]$ and an $E[X]$ are different in some sort. Essentially, $E[X]$ is a kind of multihole contexts allowing multiple occurrence of the hole and disallows name capture. So we may also write $E[\cdot]$ in the discussion, bearing in mind its difference from a general context. 
}}
...
The semantics of \HOPi\ is as below. It is worth noting that we always assume no name capture with resort to $\alpha$-conversion (e.g., in the last rule the side condition $\ve{c}\cap \fn{T_1} = \emptyset$ can be implicit). %So in the fourth rule of the first row, the usual side condition $bn(\lambda)\cap fn(T_1)=\emptyset}$ is omitted.
...
\frac{\displaystyle T_1\st{a(A)} T_1'\quad  T_2\st{(\ve{c})\overline{a}A} T_2'}{\displaystyle T_1\para T_2 \st{\tau}(\ve{c})(T_1'\,|\,T_2')}\;  \xxxx{\scriptstyle \ve{c}\,\cap\, \fn{T_1} = \emptyset} %&
\end{array}
\]
% \[\begin{array}{ll}
...
Parameterization extends the syntax and semantics of \HOPi\ as follows. %in Figure \ref{PiDn}.
...
\xxxx{We reuse the symbol $\equiv$ for the structural congruence in \HOPiDd. % (and \HOPi\ as well).} %\ \cite{SW01a}.
}
...
Now parameterization on process is obtained by taking \xiv{$\ve{U},\ve{K}$ as $\ve{X},\ve{T}$} respectively, %\ve{T'}
and parameterization on names is obtained by taking $\ve{U},\ve{K}$ as $\ve{x},\ve{m}$ respectively. The corresponding abstractions are sometimes called process abstraction and name abstraction respectively, and we may use abstraction and parameterization interchangeably. We note that in $\lrangle{\vectk{U}} T$, variables $\vectk{U}$ are bound. For convenience, names are sometimes handled in a similar way as that in \FOPi, so are the related notations. 
\xxxx{That is, we may distinguish between name variables and name constants. For example, we use name variable $x$ in the abstraction $\lrangle{x}T$ while name constant $d$ in the application $T\lrangle{d}$.
}
We denote by \HOPiDd\ the calculus \HOPi\ extended with both kinds of parameterizations.

\xxxx{Calculus \HOPiDd\ can be made more precise with the help of a type system \cite{San92},
to ensure correct use of abstraction and application. For example, $A$ should be an abstraction in $A\lrangle{P}$, a term like $\lrangle{X}P \para Q$ is ill-formed, and etc..
 This however is not important for this work, %and not presented, 
 and such a type system can be referred to Sangiorgi's work \cite{San92} (Section 2.3). 
 We always assume type consistency and typing error shall not occur whatsoever. 
%When defining HO processes with parameterization, a type system is necessary  However, this paper mainly focuses on expressiveness, and such a system is relatively not so important. (lest) It can indeed be referred to Sangiorgi's work. And so it is assumed that type errors would not happen whatsoever (based on the implicit type system).
Subject to type consistency, terms of the form $\lrangle{\ve{X}}T$ and $\lrangle{\ve{x}}T$ are referred to as process abstractions and name abstractions, and processes as those without the (outermost) domination of abstraction operations (up-to structural congruence).
}

\xxx{We further remark that the choice of modelling applications as part of the structural congruence evolves from Sangiorgi's early work. We notice that here the application over an abstraction is not modeled as a reduction (or internal action).
}
%\xx{$<$more remark from response here$>$?}
\xxxx{From certain viewpoint, a term may have an infinite application sequence (substitution), such as $\Omega\DEF (A)\lrangle{A}$ in which $A\DEF \lrangle{X}(X\lrangle{X})$ \xiv{(whose type should be a recursive one, but the specific form is not very interesting for this work).}
Yet it appears a bit different from that in lambda-calculus, where an application indeed leads to a reduction.
So this shall not be a great concern for the work here, though in general it needs some regulation, that is, the substitution should be well-defined and terminating. Interested readers can refer to Sangiorgi's seminal work that explains the typing and substitution issues \cite{San92} (Definition 2.3.3 and Lemma 2.3.4).
}
...
A symmetric relation $\mathcal{R}$ on (closed) \HOPiDd\ processes is a context bisimulation, if $P\,\mathcal{R}\, Q$ implies the following properties:
\begin{itemize}
...
\item if $P \st{(\ve{c})\overline{a}A} P'$ in which $A$ is a process abstraction, a name abstraction or not an abstraction, then $Q \wt{(\ve{d})\overline{a}B} Q'$ for some $B$ that is respectively a process abstraction, a name abstraction or not an abstraction, and moreover for every $E[X]$ such that $\{\ve{c},\ve{d}\}\cap \fn{E}=\emptyset$ it holds that 
\begin{equation}
(\ve{c})(E[A]\para P') \; \mathcal{R}\;  (\ve{d})(E[B]\para Q'). \nonumber\tag{*}%\eqnumber{\triangle}
\end{equation}
\end{itemize}
Context bisimilarity, written $\WCB$, is the largest context bisimulation.
\end{definition}

Relation $\SCB$ denotes the strong context bisimilarity. As is well-known, $\WCB$ is a congruence; see \cite{San92,San94} for a proof. 
\xiv{We stress that the bisimulations are defined over closed processes. So the expressions in (*) above in Definition \ref{context-bisimulation} should be closed in order to be relared by $\mathcal{R}$. This situation also applies to other definitions concerning bisimulations, e.g., bisimulations up-to context.
}
The extension of $\WCB$ to open processes and abstractions is defined in the expected way: suppose $\ve{X}\subseteq \fpv{T,T'}$, then $T\WCB T'$ iff $T\hosub{\ve{A}}{\ve{X}} \WCB T'\hosub{\ve{A}}{\ve{X}}$ for all closed $\ve{A}$; $\lrangle{\ve{X}}T \WCB \lrangle{\ve{X}}T'$ iff $T\hosub{\ve{A}}{\ve{X}} \WCB T'\hosub{\ve{A}}{\ve{X}}$ for all closed $\ve{A}$.
\sepp

\xxxx{We note that the matching for output in context bisimulation is required to bear the same kind of communicated process as compared to the simulated action, and the resulting two terms in (*) should be processes so as to be in $\R$ again. This is standard in such definitions; see \cite{San92}. 
%Also relavant to this, in the bisimulation, the communicated processes are required to tbe the same type, and of course type error shall not occur. 
Since we always assume type consistency, %we require the outputted terms, say A and B, in the (context) bisimulation to bear the same kind. 
it means that, for example, for the outputted terms $A$ and $B$, if $A$ is a name abstraction, so is $B$; %and also $X$ in $E[X]$.
similar for other cases. However, we do not have further requirement on the contents on these abstractions, as long as the requirement of the bisimulation is satisfied (actually this is part of the flexibility context bisimulation permits). 
Also this design somewhat simplifies discussion, e.g., the characterization by normal bisimulation. Perhaps this can be somehow relaxed, but we do not know how to do it now.
%- About requiring same type of A and B in the output clause of context bisimulation: 
}



\subsubsection{Bisimulation up-to}
\xxx{
Before going ahead, %on to the next section, 
we make some discussion about the bisimulation proof method, since it is needed during building bisimulation in this work. 
}
...
\xxrmcolor{
Similarly, a relation \R is a context bisimulation up-to context if it satisfies all the clauses of the context bisimulation, except that the resulting two processes originally related by $\R$, say $P' \,\R\, Q'$, in the conclusion of each clause can now be written as $P'\equiv C[P''], Q'\equiv C[Q'']$ for some \xxxx{context $C$, and $P''$ and $Q''$} such that $P'' \,\R\, Q''$. That is, they can be rewritten into forms that extract the common parts of $P',Q'$ into a context, and separate the possibly different sub-processes $P'',Q''$ to be indeed related by \R (the common context may also have multiple holes, in which case the processes filling in the holes are bound to be related component-wise). 
}

\xxrmcolor{
A relation \R as such above is not necessarily a bisimulation already, however, it is certain that it is subsumed by the largest bisimulation, i.e., bisimilarity. 
...
\xxxx{Below we formally define these up-to techniques for context bisimulation that are used in this work. 
}
%We do not give further details since it is not the main topic of this work; 
More systematic introduction, comprehensive discussion and development of this technique can be found in \cite{SW01a,SR12,San12,SKS11,Mil89,MPS14,DHS18}.
}
%\nts{\fbox{TODO...check the notes on the first page} }
\xiv{We recall that by default we deal with closed processes in bisimulations.}
...
\item if $P \st{(\ve{c})\overline{a}A} P'$, then $Q \wt{(\ve{d})\overline{a}B} Q'$ where $B$ and $A$ are both process abstractions,  name abstractions or not abstractions, and moreover for every $E[X]$ such that $\{\ve{c},\ve{d}\}\cap \fn{E}=\emptyset$ it holds that $(\ve{c})(E[A]\para P') \; \SCB\mathcal{R}\SCB \;  (\ve{d})(E[B]\para Q')$.
\end{itemize}
\end{definition}
...
\begin{itemize}
\item if $P \st{\alpha} P'$ and $\alpha$ is $a(A)$ or $\tau$, then $Q \wt{\widehat{\alpha}} Q'$ for some $Q'$, $P'\equiv C[P'']$ and $Q'\equiv C[Q'']$ for some \xiv{(closed)} $P'', Q''$ and context $C$, and $P''\,\mathcal{R}\, Q''$;
\item if $P \st{(\ve{c})\overline{a}A} P'$, then $Q \wt{(\ve{d})\overline{a}B} Q'$ 
%for some $B$ that is of the same type as $A$, i.e., a process abstraction, a name abstraction or not an abstraction, 
where $B$ and $A$ are both process abstractions,  name abstractions or not abstractions, 
and moreover for every $E[X]$ such that $\{\ve{c},\ve{d}\}\cap \fn{E}=\emptyset$ it holds that $(\ve{c})(E[A]\para P') \equiv C[P'']$ and $(\ve{d})(E[B]\para Q')\equiv C[Q'']$ for some \xiv{(closed)} $P'', Q''$ and context $C$, and $P''\,\mathcal{R}\, Q''$.
\end{itemize}
\end{definition}
}

\xxxx{
The two up-to techniques above can be combined, i.e., replacing $P''\,\R\, Q''$ in Definition \ref{d:up2} with $P'' \,\SCB\R\SCB\, Q''$ 
%(respectively $P'' \,\equiv\R\equiv\, Q''$) 
gives a context bisimulation up-to context and \SCB 
%(respectively $\equiv$). 
}
...
\xxxx{One may not be so confident of the soundness of up-to context as that of up-to $\equiv$ or $\SCB$, which after all can be proven in a way similar to that for first-order processes (see \cite{SW01a}). \xxxx{We prove this theorem in Appendix \ref{appendix:up-to-context_general}}.}
\xxxx{
Intuitively, the up-to context technique for higher-order processes, as well as other up-to techniques, are sound because higher-order processes can be translated into first-order ones in a fully abstract way, and these up-to techniques are sound for first-order processes \cite{San92, SW01a, MPS14}. % (which we may also appeal to while discussing the encodings). 
...
\xxxx{Further to our need, in Section \ref{s:encoding} we will also prove the soundness of the up-to context technique on top of the encodings presented in that section. As one shall see, that proof indeed is closely related to the proof of the soundness of the general up-to context technique defined in Definition \ref{d:up2}.
}
...
We \xxx{give} a notion of encoding in this section. %First, we define what
...
Notation $\encode{\mathcal{P}_1}{}{}$ stands for the image of the $\mathcal{L}_1$-processes inside $\mathcal{L}_2$ under the encoding, \xxxx{that is, $\encode{\mathcal{P}_1}{}{} \DEF \{\encoding{P}{}{} \,|\, P \mbox{ is an $\mathcal{L}_1$-process}\}$}. It should be clear that  $\encode{\mathcal{P}_1}{}{}\subseteq \mathcal{P}_2$ \cite{Gor09,Gor08a}. 

We use $\dot\approx_2$ to denote the behavioural equivalence $\approx_2$ restricted to image $\encode{\mathcal{P}_1}{}{}$ {(\xxxx{up-to the standard structural congruence})}, \xiv{with the related terms  restricted to those on that image. Since the encoding is compositional, it makes sense to assume that a context is also mapped to a context.} 
\xiv{However, contexts in the target model can be used in a somewhat more relaxed way, which we now explain. 
A legal context here for $\dot\approx_2$ means a context in the target (higher-order) model that evaluates to a term in the image of the encoding, and thus is good for the bisimulation equality. That is, when a process is put in such a context, one gets in return a term that lies in the image of the encoding. 
Usually a (receiving) context can be the context corresponding to a context in the source model run  through the encoding. Yet from the viewpoint of (context) bisimulation, a context in the (target) model can be a bit more versatile so as to allow more flexibility and discrimination. 
Consequently, it may be a context $C[\cdot]$ that is not an  encoding but evaluates, when fed with a process, to a term falling within the image of encoding (such that suitable for the bisimulation). %, i.e., within the image of the encoding. 
}
...
(1a) Adequacy. \emph{$P \approx_1 P'$ implies $\encoding{P}{}{} \approx_2 \encoding{P'}{}{}$. }
Adequacy is also known as \emph{soundness}. 
(1b) \xiv{Completeness.} 
\xxxx{
\xiv{The converse of soundness, i.e., \emph{$\encoding{P}{}{} \approx_2 \encoding{P'}{}{}$ implies $P \approx_1 P'$}, is the \emph{completeness}.
\emph{Full abstraction} refers to both soundness and completeness.}
}
(1c) Weak adequacy (or weak soundness). \emph{$P \approx_1 P'$ implies $\encoding{P}{}{} \dot\approx_2 \encoding{P'}{}{}$;} ~
...
\xxxx{
After all, we are more interested in full abstraction, and it is built on the standard bisimulation equalities %in the source and target models, 
rather than arbitrary ones as discussed in \cite{Par16} (so the full abstraction is not trivial).
}
That said, the soundness and completeness are likely based on a different manner of operational correspondence. 
}
Notwithstanding, we will discuss the operational correspondence of the encoding in this work, and moreover, as will be seen, the concrete operational correspondence therein somehow strengthens the criteria of operational correspondence %(and related concepts) 
used in \cite{Gor08a,LPSS10} (in \cite{Gor08a} the criteria are not action-labelled and thus the notion of success sensitiveness is contrived; in \cite{LPSS10} a labelled variant criteria is posited to its purpose). %success sensitiveness
