...
%}
\xxx{
We further notice that the up-to technique for $\WWCB$ is defined in the same way as that in Definition \ref{d:up1}, \ref{d:up2}, except that the communicated processes and contexts are all from images of the encoding. % (see Definition \ref{d:up3_encoding}). 
The following proposition shows the correctness of this up-to context technique, and its proof 
...
\xxx{We note that in the input and out, we do not consider general processes, but instead those sendable by an encoding, because we confine to the image of the encoding. This is where this proof cannot be extended to the general case, otherwise the counterexample above would become in vain.}
\begin{itemize}
