...
\begin{itemize}
\item The proof can be readily adapted to the proof of the correctness of the up-to context technique of general higher-order processes, for other bisimulations like normal bisimulation. Moreover, it can be specialized to the up-to context technique for the encodings. The main difference is that the (receiving) contexts are restricted and the input processes are limited. Actually nevertheless, we provide somewhat a more direct proof in the case of the technique on the encodings (Proposition \ref{prop:up-to-context_encodings}).  In the case of normal bisimulation, for instance, the input objects are triggers and the receiving context are the corresponding trigger-receiving ones. 
...
Actually this indeed leads to the congruence of the context bisimulation \cite{San92,SW01a}. % on top of encodings (as well as on general higher-order processes) is a . 
\end{itemize}
