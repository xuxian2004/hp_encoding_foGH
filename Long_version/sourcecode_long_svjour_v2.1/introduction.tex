...
In addition, we also demonstrate and analyze a variant encoding of first-order processes using only parameterization on names. That encoding exhibits a different encoding strategy and consequently quite different properties (somewhat beyond expectation), which in a sense stresses the importance of the interwork between the two kinds of parameterization. 
...
The idea of the encodings in this paper is quite different from the abovementioned works in the literature, i.e. roughly, they make good/tricky use of parameterizations to achieve delivering a name, and this method may potentially offer (technical) reference to relevant work. 
The analysis of the encodings, however, is not so trivial. So besides the ideas of the encoding which are not reported so far to our knowledge, the exploration of the encodings provides some analytical technique for related work as well.
