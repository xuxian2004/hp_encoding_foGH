...
Now parameterization on process is obtained by taking $\ve{U},\ve{K}$ as $\ve{X},\ve{T'}$ respectively, and parameterization on names is obtained by taking $\ve{U},\ve{K}$ as $\ve{x},\ve{m}$ respectively. The corresponding abstractions are sometimes called process abstraction and name abstraction respectively, and we may use abstraction and parameterization interchangeably. We note that in $\lrangle{\vectk{U}} T$, variables $\vectk{U}$ are bound. For convenience, names are sometimes handled in a similar way as that in \FOPi, so are the related notations. 
...
\xxxx{Although from certain viewpoint, a term may have an infinite application sequence (substitution), such as $\Omega\DEF (A)\lrangle{A}$ in which $A\DEF \lrangle{X}(X\lrangle{X})$, this appears a bit different from that in lambda-calculus, where an application indeed leads to a reduction.
...
Relation $\SCB$ denotes the strong context bisimilarity. As is well-known, $\WCB$ is a congruence \cite{San92,San94}. The extension of $\WCB$ to open processes and abstractions is defined in the expected way: suppose $\ve{X}\subseteq \fpv{T,T'}$, then $T\WCB T'$ iff $T\hosub{\ve{A}}{\ve{X}} \WCB T'\hosub{\ve{A}}{\ve{X}}$ for all closed $\ve{A}$; $\lrangle{\ve{X}}T \WCB \lrangle{\ve{X}}T'$ iff $T\hosub{\ve{A}}{\ve{X}} \WCB T'\hosub{\ve{A}}{\ve{X}}$ for all closed $\ve{A}$.
\sepp
...
and moreover for every $E[X]$ such that $\{\ve{c},\ve{d}\}\cap \fn{E}=\emptyset$ it holds that $(\ve{c})(E[A]\para P') \equiv C[P'']$ and $(\ve{d})(E[B]\para Q')\equiv C[Q'']$ for some $P'', Q''$ and context $C$, and $P''\,\mathcal{R}\, Q''$.
\end{itemize}
\end{definition}
}
...
We use $\dot\approx_2$ to denote the behavioural equivalence $\approx_2$ restricted to image $\encode{\mathcal{P}_1}{}{}$ {(\xxxx{up-to the standard structural congruence})}, \xxxx{with the related contexts also restricted to those on that image. Since the encoding is compositional, it makes sense to assume that a context is also mapped to a context.}
...
(1a) Adequacy and completeness. \emph{$P \approx_1 P'$ implies $\encoding{P}{}{} \approx_2 \encoding{P'}{}{}$. This is also known as \emph{soundness}. 
\xxxx{The converse is the \emph{completeness}. Altogether, they are known as the \emph{full abstraction}.}
}
