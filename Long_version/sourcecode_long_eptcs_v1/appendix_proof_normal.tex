...
We first prove the factorization theorem. In order to do this, we need some laws as to the behavior of the trigger $\triggerd$. The counterparts concerning the other two kinds of triggers, i.e., $\trigger$ and $\triggerD$, are referred to \cite{Xu13, San92}. 
...
\[
\begin{array}{l}
 (m)(\overline{a}B_1.E_1[\triggerd] \para  !m(Z).Z\lrangle{A}) \WCB  \overline{a}B_2'.(B_2'' \para  !m(Z).Z\lrangle{A})
\end{array}
\] where $B_1\equiv E_2[\triggerd]$, $B_2'\equiv \ve{\lrangle{U}}(m)(E_2' \para  !m(Z).Z\lrangle{A})$, and $B_2''\equiv \ve{\lrangle{U'}}(m)(E_1' \para  !m(Z).Z\lrangle{A})$.
...
\[
\begin{array}{l}
 (m)(E_1[\triggerd]\para E_2[\triggerd] \para  !m(Z).Z\lrangle{A}) \WCB  (m)(E_1[\triggerd] \para  !m(Z).Z\lrangle{A}) \para (m)(E_2[\triggerd] \para  !m(Z).Z\lrangle{A})
\end{array}
\]
...
%
\noindent(1) We focus on (i) since (ii) is similar and simpler. Let $P_1\DEF (m)(\alpha.E[\triggerd] \para  !m(Z).Z\lrangle{A})$ and $Q_1\DEF \alpha.(m)(E[\triggerd] \para  !m(Z).Z\lrangle{A})$. We define $\mathcal{R}_1 \DEF \{(P_1,Q_1)\}\cup \mathrm{ID}$ in which $\mathrm{ID}\DEF \{(P,P)\}$ is the identity relation. We show that $\mathcal{R}_1$ is a strong context bisimulation up-to $\equiv$, which forces the result to be true.  We iterate the cases on $\alpha$, which is obviously the only action $P_1$ incurs. %(the situation of the other way is similar).
\begin{itemize}
\item $\alpha$ is $\tau$. Then $P_1\st{\tau} (m)(E[\triggerd] \para  !m(Z).Z\lrangle{A})$ is matched by $Q_1\st{\tau} (m)(E[\triggerd] \para  !m(Z).Z\lrangle{A})$.
%
\item $\alpha$ is $c(Y)$ for some $c$. Then $P_1\st{c(R)} (m)(E[\triggerd]\hosub{R}{Y} \para  !m(Z).Z\lrangle{A})$ is matched by \\
$Q_1\st{c(R)} ((m)(E[\triggerd] \para  !m(Z).Z\lrangle{A}))\hosub{R}{Y} \equiv (m)(E[\triggerd]\hosub{R}{Y} \para  !m(Z).Z\lrangle{A})$, because $bv(\alpha)\notin fv(A)$.
%
...
For convenience, let $P_2\DEF (m)(\overline{a}B_1.E_1[\triggerd] \para  !m(Z).Z\lrangle{A})$ and $Q_2\DEF (m)(\overline{a}B_2.E_1[\triggerd] \para  !m(Z).Z\lrangle{A})$. We define
\begin{equation}\label{eq:lemma_prop-triggers}%\label{eq:lemma_r2}
\mathcal{R}_2 \DEF \left\{\Big{(}(m)(G[B_1] \para  !m(Z).Z\lrangle{A}),(m)(G[B_2] \para  !m(Z).Z\lrangle{A})\Big{)} \,\middle|\,  \mbox{ for some $G[\cdot]$,  $E_2$, and $A$} \right\} \,\cup\,\SCB
\end{equation} We notice that $E_2$ appears in $B_1,B_2$ and $m$ is fresh, i.e., $m\notin \fn{G,E_1,E_2,A}$.
One can easily observe that $(P_2,Q_2)\in \mathcal{R}_2$. We show that $\mathcal{R}_2$ is a strong context bisimulation up-to $\SCB$ by induction on $G$. For the sake of convenience, we denote the element in $\mathcal{R}_2$ as: $P_3\DEF (m)(G[B_1] \para  !m(Z).Z\lrangle{A})$ and $Q_3\DEF (m)(G[B_2] \para  !m(Z).Z\lrangle{A})$. 
\begin{itemize}
\item $G[\cdot]$ is $[\cdot]$.
...
\[
\begin{array}{l}
P_3\st{b(A')} (m)((G_1[B_1])\hosub{A'}{Y}\para  !m(Z).Z\lrangle{A}) \equiv (m)((G_1'[B_1'])\para  !m(Z).Z\lrangle{A}) \\
\mbox{ where } G_1'\DEF G_1\hosub{A'}{Y},\; B_1'\DEF B_1\hosub{A'}{Y}\equiv (E_2[\triggerd])\hosub{A'}{Y}\equiv E_2'[\triggerd] \mbox{ for some } E_2'[X] 
\end{array}
\] So $Q_3$ simulates with the following action
\[
\begin{array}{l}
Q_3\st{b(A')} (m)((G_1[B_2])\hosub{A'}{Y}\para  !m(Z).Z\lrangle{A}) \equiv (m)((G_1'[B_2'])\para  !m(Z).Z\lrangle{A}) \\
\mbox{ where } B_2'\DEF B_2\hosub{A'}{Y}\equiv (m)(E_2'[\triggerd] \para  !m(Z).Z\lrangle{A})
\end{array}
...
There are a number of cases concerning where $\alpha$ originates from, specifically: 1) $T$; 2) $G_1$; 3) $B_1$; 4) interaction between $G_1,B_1,T$ and $!m(Z).Z\lrangle{A}$(with 5 subcases). Below we look into two cases. The remainder is similar and simpler.
\begin{itemize}
...
\[
B_1\st{(\ve{c})\overline{d}A'} B_1'\qquad\quad T\st{d(A')} T'\qquad\quad P_3\st{\tau}\SCB (m)((\ve{c})(G_1[B_1']\para T') \para  !m(Z).Z\lrangle{A}) \DEF P_3'
\] Since $d$ is not $m$, the action by $B_1$ must stem from $E_2$, i.e., $E_2[Tr_m]\st{(\ve{c})\overline{d}A'} E_2'[\triggerd]\equiv B_1'$. So $B_2\st{(\ve{c})\overline{d}A'} B_2'\equiv (m)(E_2'[\triggerd]\para !m(Z).Z\lrangle{A})$, and
\[
...
\item Second, the case $\alpha$ comes from an interaction between $B_1$ and $!m(Z).Z\lrangle{A}$. That is
\[
\begin{array}{l}
B_1\equiv E_2[\triggerd] \st{(\ve{c})\overline{m}A'} B_1' \qquad A'\equiv \lrangle{Y}(Y\lrangle{h}) \mbox{ for some } h \\
...
\end{array}
\] Moreover, because $\triggerd$ has been consumed in $B_1$, we can write $B_1'$ as $E_2'[\triggerd]$ for some $E_2'[X]$ (in which $X\notin \fpv{E_2'}$). Then
\[
...
\[
\begin{array}{lrl}
B_2\equiv (m)(E_2[\triggerd] \para  !m(Z).Z\lrangle{A}) &\st{\tau}\SCB& (m)((\ve{c})(B_1' \para  A'\lrangle{A}) \para !m(Z).Z\lrangle{A}) \\
...
\[
\begin{array}{l}
P_3\st{\tau} \SCB (m)(G_1[B_1''] \para T\para !m(Z).Z\lrangle{A})\equiv P_3'' \quad \mbox{ where }\quad B_1''\equiv E_2''[\triggerd] \\
Q_3\st{\tau} \SCB (m)(G_1[B_2'']\para T \para  !m(Z).Z\lrangle{A})\equiv Q_3'' \quad \mbox{ where }\quad B_2'' \equiv (m)(E_2''[\triggerd]\para !m(Z).Z\lrangle{A})
\end{array}
...
\mathcal{R}_3 \DEF \left\{\Big{(}G[T_1],(m)(G[T_2] \para  !m(Z).Z\lrangle{A})\Big{)} \,\middle|\,  \mbox{ for some $G[\cdot]$ and $E_1$, and } m\notin fn(G,E_1,A)\right\} \,\cup\,\SCB
\end{equation}
...
We skip the details because they are very similar to the previous part (2-3) of this lemma. %that of Lemma \ref{premise-factor-0}.
...
The proof for the others is simpler and can be referred to \cite{San92, Xu13}.
...
&\WCB & \overline{a}[(m)(E_2[\triggerd] \para  !m(Z).Z\lrangle{A})].((m)(E_1[\triggerd] \para  !m(Z).Z\lrangle{A})) & (\mbox{ind. hyp.}) \\
&\WCB & (m)(\overline{a}[(m)(E_2[\triggerd] \para  !m(Z).Z\lrangle{A})].E_1[\triggerd] \para  !m(Z).Z\lrangle{A}) & (\mbox{Lemma \ref{l:prop-triggers}(1)}) \\
&\WCB & (m)(\overline{a}E_2[\triggerd].E_1[\triggerd] \para  !m(Z).Z\lrangle{A}) & (\mbox{Lemma \ref{l:prop-triggers}(2)}) \\
\end{array}
...
%
\item $E$ is $a(Y).E_1$.
This is similar and easier than the previous case, also using Lemma \ref{l:prop-triggers}(1) and the congruence property. %, and straightforward from Lemma \ref{l:prop-triggers}(1).
%
...
\WCB & (m)(E_1[[\triggerd] \para E_2[[\triggerd] \para  !m(Z).Z\lrangle{A}) &  (\mbox{Lemma \ref{l:prop-triggers}(3)})
\end{array}
\] 
