...
We examine the properties of the variant encoding given in Section \ref{s:conclusion}. For convenience, we reuse the encoding notation $\enc{}$ and there should be no confusion in context. We first note that, similar to Lemma \ref{l:syn-pro-encoding}, the encoding comes with compositionality and divergence-reflection, and preserves name substitution (i.e., name invariant) and structural congruence; these properties will be used implicitly in the arguments of this section. We also point out that since the encoding is compositional, it makes sense to extend the definition of the encoding to be over contexts, with a whole translated to a whole.
...
It is pronounced that the operational correspondence does not hold in a direct fashion, because the encoding now has two swaps: one is between input and output; the other between synchrony and asynchrony. From a certain perspective, this encoding is more of a conceptual one, and it is unequivocally a great step in the right direction, though its analysis is slightly more tricky than the encoding in Section \ref{ss:encoding_def}.
...
\item If $P \st{a(b)} P'$, then $P\SGB (\ve{e})C[a(x).P_1]$ and $P'\equiv (\ve{e})C[P_1\fosub{b}{x}] \equiv (\ve{e})(C[0]\para P_1\fosub{b}{x})$ for some context $C$, $P_1$ and $\ve{e}$ being the bound names shared between $P_1$ and its context, and \\
$\enc{P} \st{(\ve{e})\overline{a}[\lrangle{x}(\enc{P_1})]} T\SCB \enc{C[0]}$;
\item If $P \st{\overline{a}b} P'$, then $\enc{P} \st{a(A)} T \SCB A\lrangle{b} \para \enc{P'}$;
\item If $P \st{\overline{a}(b)} P'$, then $\enc{P} \st{a(A)} T \SCB (b)(A\lrangle{b} \para \enc{P'})$; %\nts{??}
...
\item If $P \st{\tau} P'$, then $\enc{P} \st{\tau} T \SCB \enc{P'}$.
\end{enumerate}
\end{lemma}
...
$P'\equiv (\ve{e})C[P_1\fosub{b}{x}] \equiv (\ve{e})(C[0]\para P_1\fosub{b}{x})$ for some context $C$ with $\ve{e}$ being the bound names shared between $P_1$ and its context, and $T\SCB \enc{C[0]}$;
\item If $\enc{P} \st{a(A)} T$, then
\begin{enumerate}
\item[(i)] $P \st{\overline{a}b} P'$, and $T\SCB A\lrangle{b} \para \enc{P'}$, or
\item[(ii)] $P \st{\overline{a}(b)} P'$, and $T\SCB (b)(A\lrangle{b} \para \enc{P'})$; %\nts{??}
\end{enumerate}
...
\item If $\enc{P} \st{\tau} T$, then $P \st{\tau} P'$ and $T\SCB \enc{P'}$.
\end{enumerate}
\end{lemma}
...
\begin{itemize}
\item Input. We have $P\st{a(b)} P_1\fosub{b}{x} \DEF P' \equiv C[P_1\fosub{b}{x}]$ and $C[0]\para P_1\fosub{b}{x} \equiv P'$. Then $\enc{P} \equiv \overline{a}[\lrangle{x}\enc{P_1}].0 \st{\overline{a}[\lrangle{x}(\enc{P_1})]} 0 \DEF T$,  and $T\SCB \enc{C[0]} \equiv 0$.
\end{itemize}
...
\begin{itemize}
\item Input. Suppose $P_1\st{a(b)} P_1'$ (the case $P_2$ does is similar) and $P\st{a(b)} P_1'\para P_2 \equiv P'$. By ind. hyp., $P_1\SGB (\ve{e_1})C'[a(x).P_3]$ and $P_1'\equiv (\ve{e_1})C'[P_3\fosub{b}{x}] \equiv (\ve{e_1})(C'[0]\para P_3\fosub{b}{x})$ for some context $C'$, $P_3$ and $\ve{e_1}$ being the bound names shared between $P_3$ and its context within $P_1$, and $\enc{P_1} \st{(\ve{e_1})\overline{a}[\lrangle{x}(\enc{P_3})]} T_1\SCB \enc{C'[0]}$. Defining $C\DEF C'\para P_2$, we have $P\SGB (\ve{e_1})C[a(x).P_3]$ and $P'\equiv (\ve{e_1})C[P_3\fosub{b}{x}] \equiv (\ve{e_1})(C[0] \para P_3\fosub{b}{x})$. Moreover %with $\ve{e_2}$ being the bound names shared between $P_3$ and $P_2$. Moreover
\[
\begin{array}{lrl}
 &\enc{P}\equiv& \enc{P_1}\para \enc{P_2} \\
 &\st{(\ve{e_1})\overline{a}[\lrangle{x}(\enc{P_3})]}& T_1\para \enc{P_2} \DEF T \\
 &\SCB& \enc{C'[0]}\para \enc{P_2} \\
 &\equiv& \enc{C'[0]\para P_2} \equiv \enc{C[0]} %\quad \mbox{ in which }
\end{array}
\]

\item Output. Suppose $P_1 \st{\overline{a}b} P_1'$ (the case $P_2$ does is similar) and $P\st{\overline{a}b} P_1'\para P_2 \equiv P'$. By ind. hyp., $\enc{P_1} \st{a(A)} T_1$ and $T_1\SCB A\lrangle{b} \para \enc{P_1'}$. Then we have
\[
\begin{array}{lrl}
 &\enc{P}\equiv& \enc{P_1}\para \enc{P_2} \\
 &\st{a(A)}& T_1\para \enc{P_2} \DEF T\\
 &\SCB& A\lrangle{b} \para \enc{P_1'} \para \enc{P_2} \\
 &\SCB& A\lrangle{b} \para \enc{P_1'\para P_2} \equiv A\lrangle{b} \para \enc{P'}
\end{array}
\]

\item Bound output. Suppose $P_1 \st{\overline{a}(b)} P_1'$ and $b\notin \fn{P_2}$, and $P\st{\overline{a}b} P_1'\para P_2 \equiv P'$ (the case $P_2$ does is similar). By ind. hyp., $\enc{P_1} \st{a(A)} T_1$ and $T_1\SCB (b)(A\lrangle{b} \para \enc{P_1'})$. Then we have
\[
\begin{array}{lrl}
 &\enc{P}\equiv& \enc{P_1}\para \enc{P_2} \\
 &\st{a(A)}& T_1\para \enc{P_2} \DEF T \\
 &\SCB& (b)(A\lrangle{b} \para \enc{P_1'}) \para \enc{P_2} \\
 &\SCB& (b)(A\lrangle{b} \para \enc{P_1'} \para \enc{P_2}) \\ %\quad \mbox{ due to } b\notin \fn{P_2} \mbox{ and name invariance } \\
 &\SCB& (b)(A\lrangle{b} \para \enc{P_1'\para P_2}) \\
 &\equiv& (b)(A\lrangle{b} \para \enc{P'})
\end{array}
\]

\item Interaction ($\tau$). The most interesting and hard case is when $P_1$ and $P_2$ engages a communication (of a bound name). Consider, for example, the case $P_1\st{a(b)} P_1'$ and $P_2\st{\overline{a}(b)} P_2'$ ($b\notin \fn{P_2}$ w.o.l.g.), and $P\st{\tau} P'\equiv (b)(P_1'\para P_2')$. By ind. hpy., $P_1\SGB (\ve{e_1})C'[a(x).P_3]$ and $P_1'\equiv (\ve{e_1})C'[P_3\fosub{b}{x}] \equiv (\ve{e_1})(C'[0] \para P_3\fosub{b}{x})$ for some context $C'$, $P_3$ and $\ve{e_1}$ being the bound names shared between $P_3$ and its context within $P_1$, and $\enc{P_1} \st{(\ve{e_1})\overline{a}[\lrangle{x}(\enc{P_3})]} T_1\SCB \enc{C'[0]}$; $\enc{P_2} \st{a(A)} T_2$ and $T_2\SCB (b)(A\lrangle{b} \para \enc{P_2'})$. Take $A$ as $\lrangle{x}(\enc{P_3})$, and we have
\[
\begin{array}{lrl}
 &\enc{P}\equiv& \enc{P_1}\para \enc{P_2} \\
 &\st{\tau}& (\ve{e_1})(T_1 \para T_2) \DEF T \\
 &\SCB& (\ve{e_1})(\enc{C'[0]} \para (b)((\lrangle{x}(\enc{P_3}))\lrangle{b} \para \enc{P_2'})) \\
 &\SCB& (\ve{e_1})(\enc{C'[0]} \para (b)(\enc{P_3}\fosub{b}{x} \para \enc{P_2'})) \\
 &\SCB& (\ve{e_1})(\enc{C'[0]} \para (b)(\enc{P_3\fosub{b}{x}} \para \enc{P_2'})) \\
 &\SCB& (b\ve{e_1})(\enc{C'[0]} \para \enc{P_3\fosub{b}{x}} \para \enc{P_2'}) \\
 &\SCB& (b\ve{e_1})(\enc{C'[0] \para P_3\fosub{b}{x} \para P_2'}) \\
 &\SCB& (b\ve{e_1})(\enc{C'[0] \para P_3\fosub{b}{x} \para P_2'}) \\
 &\SCB& (b)(\enc{(\ve{e_1})(C'[0] \para P_3\fosub{b}{x}) \para P_2'}) \\ %\nts{???} \\
 %&\SCB& (b)(\enc{(\ve{e_1})(C'[P_3\fosub{b}{x}]) \para P_2'}) \\
 &\SCB& (b)(\enc{P_1' \para P_2'})
\end{array}
\]
\end{itemize}
...
\begin{array}{lrl}
 ...
 &\SCB& \enc{C'[0]} \equiv \enc{C[0]} %\quad \mbox{ in which }
\end{array}
\]
\item[(ii)] If $c\notin \fn{P_3}$, we set $C\DEF (c)C'$ and have $P\SGB (\ve{e_1})C[a(x).P_3]$ and $P'\equiv (\ve{e_1})C[P_3\fosub{b}{x}]\equiv (\ve{e_1})((c)C'[0] \para P_3\fosub{b}{x}) \equiv (\ve{e_1})(C[0] \para P_3\fosub{b}{x})$, and moreover
\[
\begin{array}{lrl}
 &\enc{P}\equiv& (c)\enc{P_1} \\
...
\begin{itemize}
\item[(i)] $P_1 \st{\overline{a}b} P_1'$ and $P\st{\overline{a}b} (c)P_1' \equiv P'$. By ind. hyp., $\enc{P_1} \st{a(A)} T_1\SCB A\lrangle{b} \para \enc{P_1'}$. Then we have
\[
\begin{array}{lrl}
 &\enc{P}\equiv& (c)\enc{P_1} \\
 &\st{a(A)}& (c)T_1 \DEF T\\
 &\SCB& (c)(A\lrangle{b} \para \enc{P_1'}) \\
 &\SCB& A\lrangle{b} \para (c)\enc{P_1'} \\
 &\equiv& A\lrangle{b} \para \enc{(c)P_1'} \equiv A\lrangle{b} \para \enc{P'}
\end{array}
\]
\item[(ii)] $P_1 \st{\overline{a}c} P_1'$ and $P\st{\overline{a}(c)} P_1' \equiv P'$. By ind. hyp., $\enc{P_1} \st{a(A)} T_1\SCB A\lrangle{c} \para \enc{P_1'}$. Then we have
\[
\begin{array}{lrl}
 &\enc{P}\equiv& (c)\enc{P_1} \\
 &\st{a(A)}& (c)T_1 \DEF T\\
 &\SCB& (c)(A\lrangle{c} \para \enc{P_1'}) \\
...
Suppose $P_1 \st{\overline{a}(b)} P_1'$ and $P\st{\overline{a}(b)} (c)P_1' \equiv P'$. By ind. hyp., $\enc{P_1} \st{a(A)} T_1\SCB (b)(A\lrangle{b} \para \enc{P_1'})$. Then we have
\[
\begin{array}{lrl}
 &\enc{P}\equiv& (c)\enc{P_1} \\
 &\st{a(A)}& (c)T_1 \DEF T \\
 &\SCB& (c)(b)(A\lrangle{b} \para \enc{P_1'}) \\
 &\SCB& (b)(A\lrangle{b} \para (c)\enc{P_1'}) \\
 &\equiv& (b)(A\lrangle{b} \para \enc{(c)P_1'}) \\
 &\equiv& (b)(A\lrangle{b} \para \enc{P'})
\end{array}
\]

\item Interaction ($\tau$). Suppose $P_1\st{\tau} P_1'$ and $P\st{\tau} (c)P_1'\equiv P'$. By ind. hyp., $\enc{P_1} \st{\tau} T_1 \SCB \enc{P_1'}$. Then we have
\[
\begin{array}{lrl}
 &\enc{P}\equiv& (c)\enc{P_1} \\
 &\st{\tau}& (c)T_1 \DEF T \\
...
\item $P$ is $!a(x).P_1 \SCB C[a(x).P_1]$ in which $C\DEF [\cdot]\para !a(x).P_1$.
\begin{itemize}
\item Input. We have $P \st{a(b)} P_1\fosub{b}{x}\para !a(x).P_1 \DEF P' \equiv C[P_1\fosub{b}{x}]$ and $P' \equiv C[0]\para P_1\fosub{b}{x}$. Then $\enc{P} \equiv !\overline{a}[\lrangle{x}\enc{P_1}] \st{\overline{a}[\lrangle{x}(\enc{P_1})]} 0\para !\overline{a}[\lrangle{x}\enc{P_1}] \DEF T$, and $T\SCB \enc{C[0]}$.
\end{itemize}
...
Then as shown below, $(a)(\enc{R_1}\para T)$ and $(a)(\enc{R_2}\para T)$ can be distinguished, demonstrating that $\enc{R_1}$ and $\enc{R_2}$ are not context bisimilar. Particularly, the latter can do two input on $c$, while the former cannot.
\[
\begin{array}{lrl}
...
&\equiv& (ab)((f)(b(Y).(Y\lrangle{f}\para 0)) \para (f)(b(Y).(Y\lrangle{f}\para 0)) \para \overline{b}[\lrangle{x}\enc{\overline{c}}] \para \overline{b}[\lrangle{x}\enc{\overline{c}}]) \\
...
Short of soundness, one might expect that the encoding at least has weak soundness {(as defined in Section \ref{s:preliminary})}. %(We recall that weak soundness means the soundness confined to the images of the encoding in \HOPiDd, i.e., processes in \HOPiDd\ that are the encoding of \FOPi\ processes; these processes merely communicate terms of the form $\lrangle{x}(\enc{P})$ in which $P$ is a \FOPi\ process).
We demonstrate that it is not the case. We carry out the analysis of a similar counterexample below.
...
Fix two \FOPi\ processes $R_3$ and $R_4$. That they are \ground bisimilar is in the clear. We now show that their encodings are not related by $\WWCB$.
\[
...
\]
Following the stipulation of context bisimulation, we choose $E[X] \DEF X\lrangle{e} \para X\lrangle{e}$ and have
\[
\begin{array}{rcl}
...
To see this, fix a \HOPiDd\ abstraction $B$, e.g., $\lrangle{x}0$. We then have the following chasing diagram showing the inequality above. For the sake of conciseness, we use $\cdot$ to denote certain existent processes.
\[\nsepvs{.2}
\xymatrix{
...
\]
We conclude that $\enc{R_3} \;\;\bcancel{\WWCB}\;\; \enc{R_4}$.
...
Though soundness fails, the encoding indeed has completeness. It is worth noting that we need Theorem \ref{factor-bigd-smalld} in the proof. This not only exhibits somewhat the use of that theorem, but also confers uopn in a sense the synergy of the two kinds of abstractions.
\begin{lemma}\label{l:completeness_var}
Assume $P$ is a \FOPi\ process. Then $\enc{P} \WCB \enc{Q}$ implies $P\WGB Q$.
\end{lemma}
\begin{proof}
We argue that the below defined relation $\mathcal{R}$ is a local bisimulation up-to $\SGB$.
\[
...
\] Since we already know that $T\SCB \enc{C[0]}$ and $T'\WCB \enc{C''[0]}$, we have as needed
\[
...
%By Lemma \ref{l:opcor_var},
%By Lemma \ref{l:opcor-conv_var},
By Lemma \ref{l:opcor_var}, $\enc{P} \st{a(A)} T \SCB A\lrangle{b} \para \enc{P'}$. Since $\enc{P}\WCB \enc{Q}$, we know that $\enc{Q} \wt{a(A)} T' \WCB T$. By Lemma \ref{l:opcor-conv_var}, it must be that $Q \wt{\overline{a}b} Q'$ (otherwise $\enc{P}$ and $\enc{Q}$ would be distinguishable), and $A\lrangle{b} \para \enc{Q'} \wt{} T'' \WCB T'$. Setting $A$ to be $\lrangle{x}0$, we have
\begin{eqnarray}
\enc{P} \st{a(\lrangle{x}0)} T \SCB 0 \para \enc{P'} \SCB \enc{P'} \nonumber \\ %\label{eq:com_var_2} \\
...
\[
(m)(\;(b)(\triggerd\lrangle{b} \para \enc{P'})\; \para !m(Z).Z\lrangle{A}) \;\WCB\; (m)(\;(b)(\triggerd\lrangle{b} \para \enc{Q''})\; \para !m(Z).Z\lrangle{A})
\]
...
%By Lemma \ref{l:opcor_var},
%By Lemma \ref{l:opcor-conv_var},
By Lemma \ref{l:opcor_var}, $\enc{P} \st{\tau} T \SCB \enc{P'}$. Since $\enc{P}\WCB \enc{Q}$, we know that $\enc{Q} \wt{} T' \WCB T$. By Lemma \ref{l:opcor-conv_var}, $Q \wt{} Q'$ and $T'\WCB \enc{Q'}$. We need to prove $P'\,\mathcal{R}\, Q'$, i.e., $\enc{P'} \,\WCB\, \enc{Q'}$. This is straightforward because
\[
