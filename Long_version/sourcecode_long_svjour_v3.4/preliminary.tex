...
\xxx{We always assume no name capture (i.e., a free name falling into the scope of a same bound name), with the help of $\alpha$-conversion.}
...
To facilitate bisimulation and related discussion, we denote by
$E[\ve{X}]$ (sometimes called a environment) the expression $E$ (possibly) with free variables $\ve{X}$, and $E[\ve{A}]$ stands for $E\hosub{\ve{A}}{\ve{X}}$ \xxx{respecting name-capture}. 
...
\frac{\displaystyle T_1\st{a(A)} T_1'\quad  T_2\st{(\ve{c})\overline{a}A} T_2'}{\displaystyle T_1\para T_2 \st{\tau}(\ve{c})(T_1'\,|\,T_2')}\;  \xxx{\scriptstyle \ve{c}\,\cap\, \fn{T_1} = \emptyset} %&
\end{array}
\]
% \[\begin{array}{ll}
...
\xxx{We reuse the symbol $\equiv$ for the structural congruence in \HOPiDd. % (and \HOPi\ as well).} %\ \cite{SW01a}.
}
...
\xxx{That is, we may distinguish between name variables and name constants. For example, we use name variable $x$ in the abstraction $\lrangle{x}T$ while name constant $d$ in the application $T\lrangle{d}$.
}
We denote by \HOPiDd\ the calculus \HOPi\ extended with both kinds of parameterizations.

\xxx{Calculus \HOPiDd\ can be made more precise with the help of a type system \cite{San92},
to ensure correct use of abstraction and application. For example, $A$ should be an abstraction in $A\lrangle{P}$, a term like $\lrangle{X}P \para Q$ is ill-formed, and etc..
 This however is not important for this work and not presented, and such a type system can be referred to Sangiorgi's work \cite{San92} (Section 2.3). 
...
\xxx{Although from certain viewpoint, a term may have an infinite application sequence (substitution), such as $\Omega\DEF (A)\lrangle{A}$ in which $A\DEF \lrangle{X}(X\lrangle{X})$, this appears a bit different from that in lambda-calculus, where an application indeed leads to a reduction.
...
\xxx{We note that the matching for output in context bisimulation is required to bear the same kind of communicated process as compared to the simulated action, and the resulting two terms in (*) should be processes so as to be in $\R$ again. This is standard in such definitions; see \cite{San92}. 
...
\xxx{Below we formally define these up-to techniques for context bisimulation that are used in this work. 
}
...
\xxx{One may not be so confident of the soundness of up-to context as that of up-to $\equiv$ or $\SCB$, which after all can be proven in a way similar to that for first-order processes (see \cite{SW01a}). \xxx{We prove this theorem in Appendix \ref{appendix:up-to-context_general}}.}
\xxrmcolor{
...
\xxx{Further to our need, in Section \ref{s:encoding} we will also prove the soundness of the up-to context technique on top of the encodings presented in that section. As one shall see, that proof indeed is closely related to the proof of the soundness of the general up-to context technique defined in Definition \ref{d:up2}.
}
...
Notation $\encode{\mathcal{P}_1}{}{}$ stands for the image of the $\mathcal{L}_1$-processes inside $\mathcal{L}_2$ under the encoding, \xxx{that is, $\encode{\mathcal{P}_1}{}{} \DEF \{\encoding{P}{}{} \,|\, P \mbox{ is an $\mathcal{L}_1$-process}\}$}. It should be clear that  $\encode{\mathcal{P}_1}{}{}\subseteq \mathcal{P}_2$ \cite{Gor09,Gor08a}. 

We use $\dot\approx_2$ to denote the behavioural equivalence $\approx_2$ restricted to image $\encode{\mathcal{P}_1}{}{}$ {(up-to \xxx{the standard}  structural congruence)}, \xxx{with the related contexts also restricted to those on that image. Since the encoding is compositional, it makes sense to assume that a context is also mapped to a context.}
...
\xxx{The converse is the \emph{completeness}. Altogether, they are known as the \emph{full abstraction}.}
}
...
\xxx{
After all, it appears relatively more important to have full abstraction, and the full abstraction is built on the standard bisimulation equalities %in the source and target models, 
