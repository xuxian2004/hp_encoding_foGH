...
Contexts can be extended to multihole ones in the standard way \cite{SW01a}; basically a multihole context has several holes each of which may occur a couple of times.  
To facilicate bisimulation and related discussion, we denote by
$E[\ve{X}]$ (sometimes called a environment) the expression $E$ with (possibly) free variables $\ve{X}$, and $E[\ve{A}]$ stands for $E\hosub{\ve{A}}{\ve{X}}$ \xxx{respecting name-capture}. 
A context $C[\cdot]$ and an $E[X]$ are different in some sort. Essentially, $E[X]$ is a kind of contexts allowing multiple occurrence of the hole and disallows name capture. So we may also write $E[\cdot]$ in the discussion, bearing in mind its difference from a general context. 
}
...
to ensure correct use of abstraction and application. For example, $A$ should be an abstraction in $A\lrangle{P}$, a process cannot take the form $\lrangle{X}P \para Q$, and etc..
 This however is not important for this work and not presented, such a type system can be referred to Sangiorgi's work \cite{San92} (Section 2.3). 
...
Subject to type consistency, terms of the form $\lrangle{\ve{X}}T$ and $\lrangle{\ve{x}}T$ are referred to as process abstractions and name abstractions, and processes as those without the domination of abstraction operations (up-to structural congruence).
}
...
it means that, for eample, for the outputted terms $A$ and $B$, if $A$ is a name abstraction, so is $B$; %and also $X$ in $E[X]$.
similar for other cases. However, we do not have further requirement on the contents on these abstractions, and they are subject to bisimulation (actually this is part of the flexibility context bisimulation permits). 
