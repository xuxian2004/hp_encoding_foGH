...
\]
We make a case analysis. Suppose $P\R' Q$ and notice that $P$ and $Q$ are not parameterized. We notice that we may sometimes %write context and $C[\cdot]$ and often 
...
$C[Q] \st{a(A)}  C_1[Q]\hosub{A}{X}\equiv C_2[Q]$. So we have $C_2[P] \,\R'\, C_2[Q]$ with $P\,\mathcal{R}\, Q$.
...
$C[P] \st{a(A)} R\para C_1'[P]$. So $C[Q]$ simulates by $C[Q] \st{a(A)} R\para C_1'[Q]$. Now we have for context $C_2\DEF R\para C_1'[]$ 
%(because $P$ is on a firable position)
\[
\begin{array}{lcl}
...
\item The action is from $P$. That is, $P\st{a(A)} P'$, and $C[P] \st{a(A)} (d)C_1[P']$. Since $P\,\R\, Q$, we know that $Q \wt{a(A)} Q'$ for some $Q'$, $P'\equiv C'[P'']$ and $Q'\equiv C'[Q'']$ for some $P'', Q''$ and context $C$ as stipulated, and $P''\,\mathcal{R}\, Q''$. So $C[Q]$ simulates by \\
 $C[Q] \wt{a(A)} (d)C_1[Q']$. Now we have for some context $C_2\DEF (d)C_1[C'[]]$ 
%(because $P$ is on a firable position)
\[
\begin{array}{lclcl}
...
\item The action is from $C_1$. That is, $C_1[P]\st{a(A)} C_1'[P]$, and \\
$C[P] \st{a(A)} (d)C_1'[P]$. So $C[Q]$ simulates by $C[Q] \st{a(A)} (d)C_1'[Q]$. Now we have for context $C_2\DEF (d)C_1'[]$ 
%(because $P$ is on a firable position)
\[
\begin{array}{lcl}
(d)C_1'[P] &\equiv& C_2[P]  \\
