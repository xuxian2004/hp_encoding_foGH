...
%Apparently 
The encoding is compositional and divergence-reflecting (since the encoding does not introduce any extra internal action), preserves the (free) names and structural congruence,  as stated in the follow-up lemma whose proof is standard induction. %We stress that the side conditions in the encoding is not essential.
...
\xxx{
Lemma \ref{l:opcor} gives the forward operational correspondence while Lemma \ref{l:opcor-conv} gives the backward operational correspondence. In these lemmas, 
the input cases are not stated for any process inputted on $\enc{P}$, 
because the action correspondence would become unclear if arbitrary input other than those used in the lemmas is considered, due to the somewhat richer environment of \HOPiDd. This is one reason why we argue for relinquishing uniform requirement on operational semantics in the notion of encoding (Section \ref{s:criteria}). 
We note that clause (2) of the two lemmas is based on an input that does not come from an encoding; it is provided for the sake of enabling observation from a different angle.
...
\xxx{the encoding is not sound in general.}
...
\xxx{
In particular, the example above fails the general soundness because it is possible to capture the process corresponding to $\overline{b}$ then to duplicate it, by harnessing the capability of parameterization. This is similar to some behaviors that can be seen in presence of passivation (see \cite{LSS11}, Section 2.3).	
}
...
We recall that $\WWCB$ is the $\WCB$ restricted to the image of the encoding as defined in Section \ref{s:preliminary}; or in other words context bisimulation defined over the image of the encoding. Formally we have the following definition.
}

\begin{definition}\label{def:image_contxtbisi} 
\xxx{We define $\enc{\pi}$ to be the image of the encoding, i.e., \\
$
\{\encoding{P}{}{} \,|\, P \mbox{ is a $\pi$ process} \}
$ 
Since the encoding is compositional, this can be extended to contexts, with a hole mapped to a hole.
}

\xxx{We denote by $\WWCB$ the context bisimulation over $\enc{\pi}$, that is, it is defined as in Definition \ref{context-bisimulation}, except that the domain is now the image of the encoding, for both the processes and contexts.
}
\end{definition}

\xxx{It should be clear that $\WWCB$ is well defined, due to the operational correspondence and static properties of the encoding. A crucial point here, owing to the tight operational correspondence, is that the contexts to be considered in the bisimulation are those processes in the target model that have reverse-image w.r.t. the encoding. By `tight', we mean that an encoding \HOPiDd\ process always evloves into the encoding of another \FOPi\ process, and thus rendering the restriction to the encodings sensible.
}
...
\xxx{We note that in the input and out, we do not consider general processes, but instead those sendable by an encoding, because we confine to the image of the encoding. This is where, particularly the input, this proof cannot be extended to the general case, otherwise the counterexample above would become in vain.}
\begin{itemize}
...
\end{proof}
\xxx{
\paragraph{Remark.} We caution that Lemma \ref{l:soundness} cannot be extend to general soundness mainly because the input case would become incorrect. Notice that the current clause only allows the input of `encapsulation of a name' from encodings. Moving to general process would fail the lemma, 
...
\xxx{We claim that if the matching is, say $\enc{Q}\wt{\overline{a}[\lrangle{Z}(Z\lrangle{c})]} T''$ \\
