...
Clause (1) of Theorem \ref{factor-bigd-smalld} is actually Sangiorgi's seminal work \cite{San92, SW01a}. Clause (2) is shown in \cite{Xu13}. Clause (3) depicts the factorization for abstraction on names, and is the contribution of this work.   %, can be discussed through a resembling technical routine. % almost the same as (ii). %and (i)
...
In particular, $\WNB$ can be considered as a special case of $\WCB$ when the input and receiving context of output are choosen to be as in $\WNB$.
So we only need to prove $\WNB$ implies $\WCB$.
}
...
concretely the case $P$ (or $Q$) communicates a name abstraction 
%and clause (3)(ii) of Theorem \ref{factor-bigd-smalld} applies, 
\xxx{and the case of Theorem \ref{factor-bigd-smalld} when $E$ is a non-abstraction applies,} 
%and $\ve{\lrangle{U}}$ is empty, 
%and leave out the similar (and simpler) rest.
\xxx{and leave out the other cases.}
