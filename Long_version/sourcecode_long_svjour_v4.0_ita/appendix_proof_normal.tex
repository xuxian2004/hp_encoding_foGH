...
We prove the factorization theorem (Theorem \ref{factor-bigd-smalld}) in this section. In order to do this, we need some laws as to the behaviour of the trigger $\triggerd$. The counterparts concerning the other two kinds of triggers, i.e., $\trigger$ and $\triggerD$, are referred to \cite{Xu13, San92}. We note that these triggers are of different types, so there would be no conflict.
...
\subsection{\xxxx{Proof of the congruence of the normal bisimilarity}}
In this section, we give the proof of Theorem \ref{thm:normal_congru}, that is, the congruence of the normal bisimilarity defined in Definition \ref{normal-bisi-Dd}. 
\xxx{
...
%We show that $\WNB$ is closed under the specific operations of the calculus. 
...
In the argument, $\cdot$ represents some unspecified process.
...
\]
Hence we obtain \\
$C[(\ve{c})(P'\para !m(Z).Z\lrangle{A})] \,\R\, C[(\ve{d})(Q'\para !m(Z).Z\lrangle{B})]$ 
because \\
$(\ve{c})(P'\para !m(Z).Z\lrangle{A}) \,\WNB\, (\ve{d})(Q'\para !m(Z).Z\lrangle{B})$. 
...
\item $\tau$ is from $P$. That is, $P\st{\tau} P'$, and  $C[P] \st{\tau} R\para P'$. Since $P\,\WNB\, Q$, we know that $Q\wt{} Q'$ and $P'\,\WNB\, Q'$. So $C[Q]$ simulates by $C[Q] \wt{} R\para Q'$. Because $P'\,\WNB\, Q'$, we have the following pair in $\R$. 
\[
\begin{array}{lcl}
R\para P' &\R&  R\para Q'
\end{array}
